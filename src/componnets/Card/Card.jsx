   ...
   <div className="w-full h-64 my-2  m-auto rounded-md shadow-[0px_0px_4px_gray] transition-[0.3s] ">
        ...
        <div className="px-2">
            <div className=" flex  justify-between  font-semibold my-1">
                <p>{name}</p>
                { !CartItems[id] ?
            <button className=" bg-orange-300 text-white rounded-full w-6   "  onClick={()=>addToCart(id)} >+</button>:

            <div className=" flex  justify-between   gap-3 font-semibold  rounded-2 rounded-xl bg-gray-400  ">
              <button className=" bg-orange-500 text-white rounded-full w-6 " onClick={()=>removeFromCart(id)} >-</button>
              <p >{CartItems[id]}</p>
              <button className="bg-orange-500 text-white rounded-full w-6" onClick={()=>addToCart(id)} >+</button>
